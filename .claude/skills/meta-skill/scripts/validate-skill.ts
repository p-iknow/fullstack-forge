...
function parseFrontmatter(
  content: string,
): { frontmatter: Frontmatter; rawYaml: string; body: string } | null {
...
function validateSkill(skillPath: string): ValidationResult {
  ...
  if (!existsSync(skillMdPath)) {
    return {
      valid: false,
      errors: ['SKILL.md file not found (must be exact case: SKILL.md)'],
      warnings,
      info,
    }
  }
  ...
  if (existsSync(readmePath)) {
    errors.push(
      'README.md found in skill folder — not allowed per Anthropic spec (use SKILL.md or references/ instead)',
    )
  }
  ...
  if (!frontmatter.name) {
    ...
    if (name.includes('claude') || name.includes('anthropic')) {
      errors.push(
        `'name' contains reserved word ("claude" or "anthropic" are reserved by Anthropic): ${name}`,
      )
    }
  }
  ...
  if (!frontmatter.description) {
    ...
    if (desc.length < 20) {
      warnings.push(
        'Description is too short (include [What it does] + [When to use it] + trigger phrases)',
      )
    }
    if (desc.length > 1024) {
  ...
  if (rawYaml.includes('<') || rawYaml.includes('>')) {
    errors.push(
      'Frontmatter contains XML angle brackets (< or >) — forbidden per Anthropic security spec',
    )
  }
  ...
  if (bodyLines.length > 500) {
    warnings.push(
      `SKILL.md body exceeds 500 lines (${bodyLines.length} lines) — consider splitting to references/`,
    )
  }
  ...
  if (orchestrationCount >= 3) {
    warnings.push(
      `Skill invokes ${orchestrationCount} other skills/commands — consider whether a Command or Agent should own this pipeline instead`,
    )
  }
  ...
  if (existsSync(refsDir) && statSync(refsDir).isDirectory()) {
    ...
    for (const entry of refEntries) {
      ...
      if (statSync(entryPath).isDirectory()) {
        warnings.push(
          `Deeply nested reference directory: references/${entry}/ — keep references 1 level deep`,
        )
      }
    }
  }
...
function main(): void {
  ...
  console.log(
    `Summary: ${result.errors.length} error(s), ${result.warnings.length} warning(s), ${result.info.length} info`,
  )
