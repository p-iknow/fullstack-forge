...
**Result** — `docs/prd/03-commerce-domain-policy.md`의 상품 상태 정책과 재고 동시성 정책을 독립적으로 조정할 수 있다.
...
**Result** — `docs/prd/03-commerce-domain-policy.md`의 대체 정책(허용 SKU, 가격 차이 승인)을 데이터 모델로 확장 가능한 구조가 된다.
