...
TypeSpec으로 API 계약을 정의하고, 생성된 타입을 프론트/백 모두가 소비.
...
@fullstack-forge/api-spec (.tsp 소스)
    ...
    │
    ├── 프론트: ky + TanStack Query + Suspensive + paths 타입 ──→ 타입 안전 API 호출
    ├── 백엔드: components['schemas'] 타입 ──→ 응답 계약 준수
    ├── 캐시 계층: Redis (세션 캐시)
...
`types.ts`는 커밋 대상이 아니므로, clone/branch 전환 후에는 codegen을 다시 실행해야 한다.
...
이벤트 중심 앱 구현은 [06-observability-and-reliability](../roadmap/06-observability-and-reliability.md)와 [07-admin-and-operations](../roadmap/07-admin-and-operations.md)를 기준으로 진행한다.
...
{
  "dependencies": {
    "@fullstack-forge/api-spec": "workspace:*",    // 생성된 타입 (components)
...
이 섹션을 `codegen 단일 기준(SSOT)`으로 사용한다.
...
```
1. .tsp 파일 수정 (packages/api-spec/src/)
