...
> 프론트 API 계층은 `@hey-api/openapi-ts`(generated SDK/client) + `@tanstack/react-query`(캐시/동기화) + `@suspensive/react-query`(Suspense 유틸) 조합. → [02-api-patterns](./02-api-patterns.md) 참조.
...
│   ├── lib/
│   │   ├── api/
│   │   │   ├── core.ts              # ApiClientError, readApiError
│   │   │   ├── generated-client.ts  # generated client 인스턴스 설정
│   │   │   └── index.ts             # api 모듈 공개 API
│   │   └── query-client.ts          # TanStack QueryClient
│   ├── queries/
│   │   └── auth.ts              # auth query/mutation options (앱 정책 적용)
