...
> `packages/api-spec`은 vitest 대상 아님 — OpenAPI/타입 산출물 검증은 `pnpm --filter @fullstack-forge/api-spec codegen`으로 처리.
...
```jsonc
{
  ...
  "workspaces": {
    ...
    "packages/api-spec": {
      ...
      "project": ["scripts/**/*.{ts,mjs}", "generated/openapi.yaml"],
      "ignore": ["generated/**"],
    },
...
export const config: SheriffConfig = {
  ...
  modules: {
    ...
    'packages/api-spec/scripts': 'lib:api-spec',
...
| 계약 테스트       | OpenAPI 계약 vs 실제 응답 일치                | `pnpm --filter @fullstack-forge/api-spec codegen` | -         |
...
```ts
// src/routes/orders/order-state.test.ts
...
```ts
// src/routes/health/health.test.ts
import { app } from '~/app'
...
>
> codegen 상세 운영 규칙(수동/자동 실행, stale 대응)은 [01-integration](../integration/01-integration.md)의 `Codegen 워크플로`를 단일 기준으로 사용.
