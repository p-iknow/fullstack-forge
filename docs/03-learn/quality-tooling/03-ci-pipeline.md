...
**Problem** — `@hono/zod-openapi` route schema에서 `openapi.yaml`과 `types.ts`를 생성하는데, 개발자가 schema 변경 후 codegen을 잊고 커밋하면 생성된 파일이 낡은(stale) 상태로 남는다. 이 상태에서 typecheck나 build가 성공하더라도 실제 API 계약과 코드가 불일치한다.

```bash
# 개발자가 route schema를 수정했지만 codegen을 잊은 경우
...
**Result** — stale 생성 코드가 main에 합류하는 것을 원천 차단한다. 개발자는 PR에서 "codegen을 다시 실행하고 커밋하세요"라는 명확한 피드백을 받는다. ✅ code-first route schema 워크플로의 필수 게이트.
...
architecture 레시피([04-tooling.md](../../02-architecture/base/04-tooling.md))에는 sheriff와 knip도 이 단계에 포함되어 있다:

```yaml
# architecture 기준 전체 Phase 2
- run: pnpm lint
- run: pnpm format:check
...
> **Caveat**: 현재 CI 워크플로(`ci.yml`)에는 sheriff와 knip 단계가 아직 포함되어 있지 않고, architecture 레시피([04-tooling.md](../../02-architecture/base/04-tooling.md))에만 정의되어 있다. 향후 CI에 추가할 때 Phase 2에 배치한다.
...
| `git diff --exit-code` 게이트 | route schema ↔ 생성물 불일치의 main 유입 차단 |
...
> **근거 문서**: [architecture/base/04-tooling](../../02-architecture/base/04-tooling.md)
